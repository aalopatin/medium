...
//  2. Data with objects
  ...
  carsDF_data_objects.printSchema()
...
//  4. Complex Data with kay-value objects - pivot
  ...
  carsDF_data_object_key_value.show(false)
  carsDF_data_object_key_value.printSchema()

  carsDF_data_object_key_value.flatten(Map("properties" -> pivot_properties)).show(false)
...
//  5. Complex Data with kay-value objects - collect
  ...
  carsDF_data_object_key_value.show(false)
  carsDF_data_object_key_value.flatten(Map("properties" -> map_properties_collect)).show(false)
...
//  6. Complex Data with kay-value objects - seq
  ...
  carsDF_data_object_key_value.show(false)
  carsDF_data_object_key_value.flatten(Map("properties" -> map_properties_seq)).show(false)
